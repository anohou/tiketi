...
FRONTEND_URL="${FRONTEND_URL:-$(_cfg app frontend_url)}"
CORS_ALLOWED_ORIGINS="${CORS_ALLOWED_ORIGINS:-$(_cfg app cors_allowed_origins)}"
...
BUILD_CACHE_MODE="${BUILD_CACHE_MODE:-$(_cfg build cache_mode)}"
BUILD_CACHE_REF="${BUILD_CACHE_REF:-$(_cfg build cache_ref)}"
BUILD_ALLOW_CACHE_BYPASS="${BUILD_ALLOW_CACHE_BYPASS:-$(_cfg build allow_cache_bypass)}"
...
ASSET_BUILD_ENABLED="${ASSET_BUILD_ENABLED:-$(_cfg build asset_build_enabled)}"
...
# deployment_state.*
DEPLOY_STATE_REPO="${DEPLOY_STATE_REPO:-$(_cfg deployment_state repo)}"
DEPLOY_STATE_PATH="${DEPLOY_STATE_PATH:-$(_cfg deployment_state path)}"
DEPLOY_STATE_REF="${DEPLOY_STATE_REF:-$(_cfg deployment_state ref)}"
DEPLOY_STATE_TOKEN_FILE="${DEPLOY_STATE_TOKEN_FILE:-$(_cfg deployment_state token_file)}"
...
HOST_PLATFORM_ROOT="${HOST_PLATFORM_ROOT:-$(_cfg deploy host_platform_root)}"
DEPLOY_CI_SOURCE_ROOT="${DEPLOY_CI_SOURCE_ROOT:-$(_cfg deploy ci_source_root)}"

# zero_downtime.*
ZERO_DOWNTIME_ENABLED="${ZERO_DOWNTIME_ENABLED:-$(_cfg zero_downtime enabled)}"
BLUE_GREEN_KEEP_PREVIOUS_COLOR="${BLUE_GREEN_KEEP_PREVIOUS_COLOR:-$(_cfg zero_downtime keep_previous_color)}"
DEPLOY_DRAIN_SECONDS="${DEPLOY_DRAIN_SECONDS:-$(_cfg zero_downtime deploy_drain_seconds)}"
QUEUE_DRAIN_SECONDS="${QUEUE_DRAIN_SECONDS:-$(_cfg zero_downtime queue_drain_seconds)}"
QUEUE_DRAIN_TIMEOUT_ACTION="${QUEUE_DRAIN_TIMEOUT_ACTION:-$(_cfg zero_downtime queue_drain_timeout_action)}"
HEALTHCHECK_ATTEMPTS="${HEALTHCHECK_ATTEMPTS:-$(_cfg zero_downtime healthcheck_attempts)}"
HEALTHCHECK_INTERVAL_SECONDS="${HEALTHCHECK_INTERVAL_SECONDS:-$(_cfg zero_downtime healthcheck_interval_seconds)}"
HEALTHCHECK_TIMEOUT_SECONDS="${HEALTHCHECK_TIMEOUT_SECONDS:-$(_cfg zero_downtime healthcheck_timeout_seconds)}"
TRAEFIK_FILE_RELOAD_MAX_SECONDS="${TRAEFIK_FILE_RELOAD_MAX_SECONDS:-$(_cfg zero_downtime traefik_file_reload_max_seconds)}"
ALLOW_UNSAFE_MIGRATIONS="${ALLOW_UNSAFE_MIGRATIONS:-$(_cfg zero_downtime allow_unsafe_migrations)}"
DIRECT_ORIGIN_IP="${DIRECT_ORIGIN_IP:-$(_cfg zero_downtime direct_origin_ip)}"
TENANT_SMOKE_HOSTS="${TENANT_SMOKE_HOSTS:-$(_cfg zero_downtime tenant_smoke_hosts)}"
CLOUDFLARE_CACHE_PURGE_PATHS="${CLOUDFLARE_CACHE_PURGE_PATHS:-$(_cfg zero_downtime cloudflare_cache_purge_paths)}"

ZERO_DOWNTIME_ENABLED="${ZERO_DOWNTIME_ENABLED:-false}"
BLUE_GREEN_KEEP_PREVIOUS_COLOR="${BLUE_GREEN_KEEP_PREVIOUS_COLOR:-true}"
DEPLOY_DRAIN_SECONDS="${DEPLOY_DRAIN_SECONDS:-30}"
QUEUE_DRAIN_SECONDS="${QUEUE_DRAIN_SECONDS:-60}"
QUEUE_DRAIN_TIMEOUT_ACTION="${QUEUE_DRAIN_TIMEOUT_ACTION:-abort}"
...
TRAEFIK_FILE_RELOAD_MAX_SECONDS="${TRAEFIK_FILE_RELOAD_MAX_SECONDS:-30}"
...
TRAEFIK_DYNAMIC_DIR="${TRAEFIK_DYNAMIC_DIR:-$(_cfg traefik dynamic_dir)}"
...
# ── Export everything ─────────────────────────────────────────────────────────
export APP_NAME APP_SLUG APP_URL APP_DOMAIN FRONTEND_URL CORS_ALLOWED_ORIGINS APP_IMAGE COMPOSE_PROJECT_BASE \
       ...
       BUILD_ALLOW_FULL_LOCAL_RUNTIME BUILD_CACHE_MODE BUILD_CACHE_REF BUILD_ALLOW_CACHE_BYPASS \
       BUILD_ALLOW_LOCAL_BUILD \
       ...
       DEPLOY_STATE_REPO DEPLOY_STATE_PATH DEPLOY_STATE_REF DEPLOY_STATE_TOKEN_FILE \
       DEPLOY_STRATEGY DEPLOY_ALLOW_EMERGENCY_BUILD DEPLOY_AUDIT_LOG_PATH \
       DEPLOY_RUNTIME_ROOT HOST_PLATFORM_ROOT DEPLOY_CI_SOURCE_ROOT \
       ZERO_DOWNTIME_ENABLED BLUE_GREEN_KEEP_PREVIOUS_COLOR DEPLOY_DRAIN_SECONDS \
       QUEUE_DRAIN_SECONDS QUEUE_DRAIN_TIMEOUT_ACTION HEALTHCHECK_ATTEMPTS \
