...
const hasFilters = computed(() => Boolean(search.value || selectedStationId.value || selectedVehicleId.value));
...
watch([search, selectedStationId, selectedVehicleId], () => {
...
const periodLabel = (assignment) => {
  if (!assignment.valid_from && !assignment.valid_until) return 'Permanente';
    ...
    <div class="flex h-full w-full flex-col overflow-hidden">
      ...
      <div class="grid min-h-0 flex-1 grid-cols-12 gap-4 px-6 pb-6">
        <aside class="col-span-12 h-full overflow-y-auto pr-2 md:col-span-2">
          <SettingsMenu v-if="usesSettingsMenu" />
          <FleetMenu v-else />
        </aside>
            ...
            <table v-else class="w-full min-w-[900px] border-collapse text-left">
              ...
              <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                <tr v-for="assignment in visibleAssignments" :key="assignment.id" class="group transition hover:bg-emerald-50/40 dark:hover:bg-emerald-950/10">
                  ...
                  <td class="px-4 py-3.5">
                    <span :class="assignment.active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300' : 'bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300'" class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-black">
      ...
      <template #content>
        <div class="space-y-4">
          ...
          <div v-if="!form.permanent" class="grid grid-cols-2 gap-3">
            <div><label class="mb-1 block text-xs font-bold text-slate-500">Du</label><input v-model="form.valid_from" type="date" class="w-full rounded-xl border-slate-200 text-sm dark:border-slate-700 dark:bg-slate-950" /><InputError :message="errors.valid_from" /></div>
            <div><label class="mb-1 block text-xs font-bold text-slate-500">Au</label><input v-model="form.valid_until" type="date" class="w-full rounded-xl border-slate-200 text-sm dark:border-slate-700 dark:bg-slate-950" /><InputError :message="errors.valid_until" /></div>
          </div>
          <div>
