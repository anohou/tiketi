...
<template>
  <Head title="Gestionnaire de flotte" />
  <MainNavLayout>
    <div class="max-w-7xl mx-auto space-y-6 pb-10">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-gray-500 mt-1">Gestion des véhicules, types de véhicules et affectations</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div v-for="card in statCards" :key="card.key" :class="['rounded-2xl p-5 text-white shadow-lg bg-gradient-to-br', card.tone]">
          ...
          <div class="text-sm text-white/80 mt-1">{{ card.label }}</div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <section class="xl:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div class="p-5 border-b border-slate-100 bg-gradient-to-r from-emerald-50 to-slate-50 flex items-center justify-between">
            ...
            <Link :href="route('fleet.assignments.index')" class="text-sm font-bold text-sky-700 hover:underline">Voir les affectations</Link>
          </div>
          <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Link :href="route('fleet.assignments.index')" class="rounded-2xl border border-emerald-100 bg-emerald-50/60 p-5 hover:bg-emerald-50 transition-colors">
              ...
              <div class="font-black text-gray-900">Affecter des véhicules</div>
              <div class="text-sm text-gray-500 mt-1">Attribuer des véhicules aux gestionnaires.</div>
            </Link>
          </div>
        </section>
